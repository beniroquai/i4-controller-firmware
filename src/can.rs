...
static CAN: Mutex<RefCell<Option<FdCan<FdCan2, NormalOperationMode>>>> =
...
fn transmit_can_messages(can: &mut FdCan<FdCan2, NormalOperationMode>) {
    loop {
        // Check if queue is full
        // Driver lacks API for this so go straight to register
        if pac::FDCAN2.txfqs().read().tfqf() {
            break;
        }
...
pub fn init_can() {
    ...
    const PRESC: NonZeroU8 = NonZeroU8::new(8).unwrap();
    const SEG1: NonZeroU8 = NonZeroU8::new(13).unwrap();
    ...
    pac::FDCAN2.txbtie().write(|w| w.0 = 7);
    ...
    unsafe { cortex_m::peripheral::NVIC::unmask(pac::Interrupt::FDCAN2_IT0) };
}
